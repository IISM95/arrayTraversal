...
function getUsersGeo(array, id) {
 return array.find(elem => elem.id === id).address.geo;
}
...
function getUsersCompany(array, id) {
	return array.find(element => element.id === id ? element : null)
}
...
function changePhone(array, id, phone) {
	return array.map(element => element.id === id ? {...element,phone:phone } : element)
}
console.log(changePhone(users, 1, "+79280019955"));
